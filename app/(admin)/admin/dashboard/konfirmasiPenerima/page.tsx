...
export default function KonfirmasiPenerimaPage() {
  ...
  useEffect(() => {
    fetch("/api/Pengguna/getPenerimaApprove")
      ...
      .catch((err) => {
        console.error("Error fetch shipments:", err);
        setShipments([]);
      });
  }, []);
  ...
  const executeConfirm = async (shipmentId: number, action: string) => {
    try {
      ...
      setSuccessPopupMsg(
        `${data.message} | Poin diberikan: ${data.poinDiberikan}`,
      );
                        ...
                        <div className="border border-gray-200 rounded-lg px-4 py-3 flex items-center gap-3 bg-white">
                          ...
                          <div>
                            ...
                            <button
                              ...
                              className="text-sm font-medium text-teal-600 hover:text-teal-800 hover:underline transition text-left"
                            >
                              Detail Barang dan Pengiriman
                            </button>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 mx-5 mt-3">
                        ...
                        <div className="rounded-lg px-4 py-3 flex items-center gap-3 bg-yellow-50 border border-yellow-200">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[10px] font-semibold text-yellow-700 tracking-widest">
                              NOMOR IDENTITAS (NIK)
                            </p>
                            <div className="flex items-center gap-2 mt-0.5">
                              <p
                                ...
                                style={{
                                  filter: blurred[s.id] ? "blur(6px)" : "none",
                                  userSelect: blurred[s.id] ? "none" : "text",
                                }}
                              >
                                {p?.identityId ?? "-"}
                              </p>
